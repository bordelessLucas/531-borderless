  ...
  return (
    <div>
      <header className="mb-8">
        ...
        <p className="mt-1 text-ink-muted">Fornecedores, integração e regras de repasse.</p>
      </header>
          ...
          <tbody className="divide-y divide-surface-border">
            {partners.map((p) => (
              <tr key={p.id} className="transition-colors hover:bg-surface-subtle">
                ...
                <td className="px-5 py-4 text-ink-muted">{p.defaultStrategy === "API" ? "Automática" : "Manual"}</td>
                <td className="px-5 py-4 text-ink-muted">{p.integration?.adapterKey ?? "—"}</td>
                <td className="px-5 py-4 text-ink-muted">{(p.commissionBps / 100).toFixed(1)}%</td>
                <td className="px-5 py-4">
                  <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${p.isActive ? "bg-emerald-100 text-emerald-700" : "bg-surface-subtle text-ink-subtle"}`}>
