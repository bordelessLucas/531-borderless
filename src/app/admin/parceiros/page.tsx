...
export default function AdminParceirosPage() {
  const { data: partners, error, isLoading } = useAdminData(listPartners, "parceiros");
  ...
  return (
    <div>
      <header className="mb-8 flex items-center justify-between gap-4">
        ...
        <Link href="/admin/parceiros/editor">
          <Button>
      ...
      {isLoading ? <p className="text-sm text-ink-muted">Carregando parceiros…</p> : null}
              ...
              {partners.map((p) => (
                <tr key={p.id} className="transition-colors hover:bg-surface-subtle">
                  <td className="px-5 py-4">
                    <Link
                      href={`/admin/parceiros/editor?id=${p.id}`}
